...
exports.onNewConnectionRequest = onDocumentCreated({
    ...
    try {
        ...
        const receiverId = connectionData.receiver_id || connectionData.reciever_id;
        console.log(receiverId, 'fucked')
        const cleanReceiverId = receiverId ? receiverId.replace(/"/g, '') : null;

        if (!cleanReceiverId) {
            console.error(`Connection ${connectionId} is missing receiver ID`);
            return { success: false, error: 'Missing receiver ID' };
        }
        const receiverDoc = await admin.firestore().collection('users').doc(cleanReceiverId).get();
